function honey_top90_projection_Q2(HNex, HNey, volfrac, penal, rfill, ft)
%% ----------- Material Properties ----------- 
E0 = 1;
Emin = E0 * 1e-9;
...
% Apply force F = √2 at 45° angle at the top-right corner
F_magnitude = sqrt(2);
F_x = F_magnitude * cos(pi/4);  % F*cos(45°)
F_y = F_magnitude * sin(pi/4);  % F*sin(45°)

% Find top-right node (maximum x, minimum y)
maxX = max(HoneyNCO(:,1));
minY = min(HoneyNCO(:,2));
[~, topRightNodeIdx] = min(sum([(HoneyNCO(:,1) - maxX).^2, (HoneyNCO(:,2) - minY).^2], 2));

F(2*topRightNodeIdx-1) = F_x;     % Horizontal component (positive x)
F(2*topRightNodeIdx) = -F_y;      % Vertical component (negative y)

% Fix all DOFs at the left end (cantilever beam)
leftNodes = find(abs(HoneyNCO(:,1)) < 1e-6);
fixeddofs = [2*leftNodes-1; 2*leftNodes];
fixeddofs = reshape(fixeddofs, [], 1);
