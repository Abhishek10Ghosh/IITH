% function HoneyTop90_Beam(HNex, HNey, volfrac, penal, rfill, ft)
% %% ----------- Material Properties -----------
% E0 = 1;
% Emin = E0 * 1e-9;
% 
...
% [xPhys, loop, change, maxiter, dv, move] = deal(x, 0, 1, 200, ones(Nelem,1), 0.2);
% 
% %% ----------- Optimization Loop -----------
...
% function eg1_honeyTop_Projection(HNex, HNey, volfrac, penal, rfill, ft)
% %% ----------- Material Properties ----------- 
% E0 = 1;
% Emin = E0 * 1e-9;
% 
...
% HHs = spdiags(1./sum(HHs,2), 0, Nelem, Nelem) * HHs;
% 
% %% ----------- Helmholtz Filter (Added) ----------- 
% helmholtz_filter = spdiags(1 ./ (1 + rfill * sqrt(DD(:,3)).^2), 0, Nelem, Nelem);
% HHs = helmholtz_filter * HHs;
% 
% %% ----------- Initialization ----------- 
% x = volfrac * ones(Nelem,1);
% [xPhys, loop, change, maxiter, dv, move] = deal(x, 0, 1, 200, ones(Nelem,1), 0.2);
...
%     dc = -penal * (E0-Emin) * xPhys.^(penal-1) .* ce;
% 
%     % Apply Helmholtz Filtering directly on sensitivities (replacing Sensitivity Filtering)
%     dc = HHs' * dc;
% 
%     % Optimality Criteria Update
%     xold = x;
